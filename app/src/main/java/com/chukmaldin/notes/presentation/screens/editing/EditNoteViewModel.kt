...
import com.chukmaldin.notes.domain.DeleteNoteUseCase
import com.chukmaldin.notes.domain.EditNoteUseCase
import com.chukmaldin.notes.domain.GetNoteUseCase
...
class EditNoteViewModel @AssistedInject constructor(
    ...
    private val getNodeUseCase: GetNoteUseCase,
    private val deleteNoteUseCase: DeleteNoteUseCase
) : ViewModel() {

    private val _state =
        MutableStateFlow<EditNoteState>(EditNoteState.Initial)
    val state = _state.asStateFlow()

    init {
        viewModelScope.launch {
            _state.update {
                val note = getNodeUseCase(noteId)
        ...
        when (command) {
            ...
            EditNoteCommand.Save -> {
                viewModelScope.launch {
                    _state.update { previousState ->
                        if (previousState is EditNoteState.Editing) {
                            val newNote = previousState.note.content.toMutableList()
                                .filter {
                                    it !is ContentItem.Text || it.content.isNotBlank()
                                }.let { previousState.note.copy(content = it) }
                            editNoteUseCase(note = newNote)
                ...
                viewModelScope.launch {
                    _state.update { previousState ->
                        if (previousState is EditNoteState.Editing) {
                            ...
                            deleteNoteUseCase(noteId = newNote.id)
                            EditNoteState.Finished
                        } else {
                            previousState
                        }
            ...
            is EditNoteCommand.AddImage -> {
                _state.update { previousState ->
                    if (previousState is EditNoteState.Editing) {
                        val newNote = previousState.note.content.toMutableList().apply {
                            val lastItem = last()
                            ...
                            previousState.note.copy(content = it)
                        }
                        previousState.copy(note = newNote)
                    } else {
                        previousState
